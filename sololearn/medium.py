...
def roadrunner():
    ...
    print("Meep Meep" if (50+distance)/coyote_speed >
          distance/roadrunner_speed else "Yum")
...
def carrot_cake():
    carrots = int(input())
    boxes = int(input())
    print("Cake Time" if carrots %
          boxes >= 7 else "I need to buy {} more".format(7 - carrots % boxes))
...
def name_buddy():
    names = input().split()
    my_name = input()
    print("Compare notes" if any([my_name[0] == s[0]
                                  for s in names]) else "No such luck")
...
def divisible():
    ...
    print("divisible by all" if all(
        [my_int % i == 0 for i in ints]) else "not divisible by all")
...
def tax_free():
    prices = [float(x) if float(x) > 20 else float(x)
              * 1.07 for x in input().split(',')]
...
@challenge(name="Missing Numbers",
           level="Medium",
           desc="""
        Create a program that takes in a list of 
        numbers and outputs the missing numbers in
        the sequence separated by spaces."""
           )
def missing_numbers():
    n = int(input())
    numbers = []
    ...
    print(" ".join([str(x)
                    for x in range(numbers[0], numbers[-1]) if x not in numbers]))


@challenge(name="Initials",
           level="Medium",
           desc="""
        Given a list of first and last names, take the
        first letter from each name to create initials
...
def initials():
    ...
    for i in range(n):
        names.append(input())
    print(" ".join([n.split()[0][0]+n.split()[1][0] for n in names]))


@challenge(name="Credit Card Validator",
           level="Medium",
           desc="""
        Given a credit card number, validate the it is
        valid using the Luhn test. Also, all valid cards
        must have exactly 16 digits."""
           )
def credit_card_validator():
    ccn = input()
    if re.match(r"^[0-9]{16}$", ccn):
        # Lunh Test
        # 1. Reverse the ccn
        ccn = ccn[::-1]
        # 2. Multiple every second digit by 2
        ccn_multiplied = [(int(c)*2 if i % 2 == 1 else int(c))
                          for i, c in enumerate(ccn)]
        # 3. Substract 9 from all numbers higher than 9
        ccn_multiplied = [(c - 9 if c > 9 else c) for c in ccn_multiplied]
        # 4. Add all the digits together
        summ = sum(ccn_multiplied)
        # 5. Modulo 10 of that sum shoul be equal to 0
        print("valid" if summ % 10 == 0 else "not valid")
    else:
        print("not valid")
...
@challenge(name="CMYK to RGB",
           level="Medium",
           desc="""
        GIven a color in CMYK format output the
        corresponding RGB color."""
           )
def cmyk_to_rgb():
    c, m, y, k = [float(input()) for x in range(4)]
    r = round(255 * (1-c) * (1-k))
    g = round(255 * (1-m) * (1-k))
    b = round(255 * (1-y) * (1-k))
    print("{},{},{}".format(r, g, b))
...
def snap_crackle_pop():
    ...
    def f(x): return "Pop" if not x % 3 else (
        "Snap" if not x % 2 else "Crackle")
...
def duty_free():
    print('Back to the store' if any(
        [float(x)*1.1 > 20 for x in input().split()]) else 'On to the terminal')
...
def no_numerals():
    p = input()
    repl = {10: "ten", 0: "zero", 1: "one", 2: "two", 3: "three",
...
def secret_message():
    ...
    phrase = input().lower()
    print("".join((alp_rev[alpha.index(i)] if i in alpha else i)
                  for i in phrase))
